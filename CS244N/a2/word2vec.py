...
def naiveSoftmaxLossAndGradient(
    ...
    # loss = -np.log(softmax(outsideVectors[outsideWordIdx].dot(centerWordVec)))
    ...
    gradCenterVec = np.dot(y_pred, outsideVectors)
    gradOutsideVecs = y_pred[np.newaxis, :].T.dot(centerWordVec[np.newaxis, :])
    # gradOutsideVecs[outsideWordIdx] = centerWordVec.dot(y-y_pred[outsideWordIdx])
        ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for Oword in outsideWords:

        l, gradC, gradO = word2vecLossAndGradient(centerWordVec, word2Ind[Oword], outsideVectors, dataset)
        #print(gradC.shape, gradCenterVecs.shape)
        loss += l
